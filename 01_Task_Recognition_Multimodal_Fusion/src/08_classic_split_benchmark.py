...
if IS_COLAB:
    ...
    # Check work-level output directory first, then fallback to repo root
    if (SCRIPT_DIR.parent / 'output' / 'research_outputs').exists():
        ROOT = SCRIPT_DIR.parent / 'output'
    elif (SCRIPT_DIR.parents[1] / 'output' / 'research_outputs').exists():
        ROOT = SCRIPT_DIR.parents[1] / 'output'
    else:
        ROOT = SCRIPT_DIR.parent / 'output'
